...
echo "alias sudo='sudo -E env PATH=/usr/local/bin:$PATH'" >> /home/vagrant/.bashrc
...
sudo /usr/local/bin/kubectl wait --for=condition=available --timeout=600s deployments --all -n argocd
